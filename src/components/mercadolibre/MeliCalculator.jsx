    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      >
        <form onSubmit={handleCalculate} className="flex flex-col gap-4 text-xs">
          ...
          {/* Mercado Envíos */}
          <div className="p-4 bg-[#faf9f5] border border-[#e5e3dc] rounded-2xl flex flex-col gap-3">
            <label className="flex items-center gap-2.5 cursor-pointer text-xs font-bold text-[#141413]">
              <input 
                type="checkbox"
            ...
            {calcForm.has_free_shipping && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-[#ece9df]">
                <Input
      ...
      >
        {calcResult ? (
          <div className="flex flex-col gap-4 text-xs">
            {/* Resumen Principal */}
            <div className="p-4 bg-[#faf9f5] border border-[#e5e3dc] rounded-2xl flex items-center justify-between">
              <div>
                <span className="text-[10.5px] font-bold text-[#73726c] uppercase tracking-wider block">
                  Precio Publicado Sugerido
                </span>
                <span className="text-2xl font-black font-mono text-[#141413]">
                  ${calcResult.precio_publicado_sugerido?.toLocaleString()}
                </span>
              </div>
              <div className="text-right">
                <span className="text-[10.5px] font-bold text-[#15803d] uppercase tracking-wider block">
                  Neto en Mano
                </span>
            ...
            <div className="border border-[#e5e3dc] rounded-2xl divide-y divide-[#ece9df] text-xs">
              <div className="p-3 flex justify-between items-center">
                <span className="text-[#73726c]">Costo Base / Mostrador ERP:</span>
                <span className="font-bold font-mono text-[#141413]">${calcResult.costo_efectivo?.toLocaleString()}</span>
              </div>
              <div className="p-3 flex justify-between items-center">
                <span className="text-[#73726c]">Comisión Mercado Libre ({calcResult.comision_porcentaje}% + fija):</span>
                <span className="font-bold font-mono text-[#b91c1c]">-${calcResult.comision_total_monto?.toLocaleString()}</span>
              </div>
              <div className="p-3 flex justify-between items-center">
                <span className="text-[#73726c]">Costo Envío Vendedor (Bonif. {calcResult.bonificacion_envio_pct}%):</span>
                <span className="font-bold font-mono text-[#b91c1c]">-${calcResult.costo_envio_final?.toLocaleString()}</span>
              </div>
              <div className="p-3 flex justify-between items-center">
                <span className="text-[#73726c]">Impuestos / Percepciones ({calcResult.alicuota_impuestos_pct}%):</span>
                <span className="font-bold font-mono text-[#b91c1c]">-${calcResult.impuestos_monto?.toLocaleString()}</span>
              </div>
            </div>

            <div className="p-3.5 bg-[#f0fdf4] border border-[#bbf7d0] rounded-2xl text-[11px] text-[#14532d] flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-[#15803d] shrink-0" />
              <span>El precio calculado absorbe las comisiones y gastos garantizando el neto requerido.</span>
            </div>
          </div>
        ) : (
          <div className="py-16 text-center text-[#73726c] text-xs flex flex-col items-center gap-3">
            <Calculator className="w-10 h-10 text-[#9c998f]" />
